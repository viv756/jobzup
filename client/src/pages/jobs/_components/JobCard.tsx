  ...
  return (
    <div className=" max-w-[450px] overflow-hidden rounded-xl bg-gray3 hover:bg-white shadow p-6 border border-secondary2">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-[26px] font-roboto font-semibold">{job.company.companyName}</h1>
      </div>
      <div className=" flex flex-col gap-3 mt-7">
        <h3 className="text-xl text-gray-900 font-semibold">{job.title}</h3>
        ...
        <Link
          to={`/job/${job._id}`}
          className="btn rounded-3xl text-lg bg-black text-white p-6 w-48 hover:bg-primary mt-5">
          View details
        </Link>
      </div>
    </div>
